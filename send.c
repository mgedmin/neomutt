                    ...
                    struct Context *ctx, struct Header *cur)
{
  ...
  if (!(flags & (SENDPOSTPONED | SENDRESEND)) &&
      !((flags & SENDDRAFTFILE) && ResumeDraftFiles))
  {
    ...
    if (SigOnTop && !(flags & (SENDMAILX | SENDKEY | SENDBATCH)) &&
      ...
      !(flags & (SENDBATCH | SENDMAILX | SENDPOSTPONED | SENDRESEND)))
  {
    ...
    if (msg->security || CryptOpportunisticEncrypt)
    {
      ...
      if (cur)
      {
        if ((WithCrypto & APPLICATION_PGP) && CryptAutopgp &&
      ...
      if (!(msg->security & (APPLICATION_SMIME | APPLICATION_PGP)))
      {
        if ((WithCrypto & APPLICATION_SMIME) && CryptAutosmime &&
            SmimeIsDefault)
          msg->security |= APPLICATION_SMIME;
