...
int imap_wait_keepalive(pid_t pid)
{
  ...
  short imap_passive = OPT_IMAP_PASSIVE;
...
int imap_account_match(const struct Account0 *a1, const struct Account0 *a2)
{
  ...
  const struct Account0 *a1_canon = a1_idata == NULL ? a1 : &a1_idata->conn->account;
  const struct Account0 *a2_canon = a2_idata == NULL ? a2 : &a2_idata->conn->account;
